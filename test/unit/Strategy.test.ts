...
describe("Base-Offchain Strategy", () => {
    ...
    describe("OffChainStrategy", () => {
        ...
        xit("maxRedeem returns correct min of idle and shares", async () => {
            await strategy.setVault(governance.address);
            await usdc.connect(governance).transfer(strategy.getAddress(), amount);
            await strategy.connect(governance).deposit(amount, henry.address);
            const shares = await strategy.balanceOf(henry.address);
            const redeemable = await strategy.maxRedeem(henry.address);
            console.log("Shares owned by Henry:", shares.toString());
            console.log("Max redeem shares:", redeemable.toString());
            console.log("Total idle:", (await strategy.totalIdle()).toString());
            expect(redeemable).to.equal(shares);
            await usdc.connect(governance).mint(strategy.getAddress(), amount);
            await strategy.connect(bob).invest(amount / 2n);
            const redeemableAfterInvest = await strategy.maxRedeem(henry.address);
            console.log("Max redeem shares after invest:", redeemableAfterInvest.toString());
            expect(redeemableAfterInvest).to.equal(amount / 2n);
        });
